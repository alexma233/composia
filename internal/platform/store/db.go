...
func (db *DB) migrate(ctx context.Context) error {
	migrations := []sqliteMigration{{
		...
		version: 4,
		statements: []string{
			"CREATE TRIGGER IF NOT EXISTS trg_tasks_validate_insert BEFORE INSERT ON tasks FOR EACH ROW SELECT CASE WHEN NEW.type NOT IN ('deploy', 'stop', 'restart', 'update', 'backup', 'restore', 'migrate', 'migrate_rollback', 'dns_update', 'caddy_sync', 'caddy_reload', 'image_check', 'prune', 'rustic_init', 'rustic_forget', 'rustic_prune', 'docker_start', 'docker_stop', 'docker_restart', 'docker_remove') THEN RAISE(ABORT, 'invalid task type') WHEN NEW.source NOT IN ('web', 'cli', 'others', 'schedule', 'system', 'auto_deploy') THEN RAISE(ABORT, 'invalid task source') WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid task status') WHEN NEW.params_json IS NOT NULL AND json_valid(NEW.params_json) = 0 THEN RAISE(ABORT, 'invalid task params_json') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_tasks_validate_update BEFORE UPDATE OF type, source, status, params_json ON tasks FOR EACH ROW SELECT CASE WHEN NEW.type NOT IN ('deploy', 'stop', 'restart', 'update', 'backup', 'restore', 'migrate', 'migrate_rollback', 'dns_update', 'caddy_sync', 'caddy_reload', 'image_check', 'prune', 'rustic_init', 'rustic_forget', 'rustic_prune', 'docker_start', 'docker_stop', 'docker_restart', 'docker_remove') THEN RAISE(ABORT, 'invalid task type') WHEN NEW.source NOT IN ('web', 'cli', 'others', 'schedule', 'system', 'auto_deploy') THEN RAISE(ABORT, 'invalid task source') WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid task status') WHEN NEW.params_json IS NOT NULL AND json_valid(NEW.params_json) = 0 THEN RAISE(ABORT, 'invalid task params_json') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_task_steps_validate_insert BEFORE INSERT ON task_steps FOR EACH ROW SELECT CASE WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid task step status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_task_steps_validate_update BEFORE UPDATE OF status ON task_steps FOR EACH ROW SELECT CASE WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid task step status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_backups_validate_insert BEFORE INSERT ON backups FOR EACH ROW SELECT CASE WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid backup status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_backups_validate_update BEFORE UPDATE OF status ON backups FOR EACH ROW SELECT CASE WHEN NEW.status NOT IN ('pending', 'running', 'awaiting_confirmation', 'succeeded', 'failed', 'cancelled') THEN RAISE(ABORT, 'invalid backup status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_services_validate_insert BEFORE INSERT ON services FOR EACH ROW SELECT CASE WHEN NEW.is_declared NOT IN (0, 1) THEN RAISE(ABORT, 'invalid services.is_declared') WHEN NEW.runtime_status NOT IN ('running', 'stopped', 'error', 'unknown') THEN RAISE(ABORT, 'invalid service runtime_status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_services_validate_update BEFORE UPDATE OF is_declared, runtime_status ON services FOR EACH ROW SELECT CASE WHEN NEW.is_declared NOT IN (0, 1) THEN RAISE(ABORT, 'invalid services.is_declared') WHEN NEW.runtime_status NOT IN ('running', 'stopped', 'error', 'unknown') THEN RAISE(ABORT, 'invalid service runtime_status') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_service_instances_validate_insert BEFORE INSERT ON service_instances FOR EACH ROW SELECT CASE WHEN NEW.is_declared NOT IN (0, 1) THEN RAISE(ABORT, 'invalid service_instances.is_declared') WHEN NEW.runtime_status NOT IN ('running', 'stopped', 'error', 'unknown') THEN RAISE(ABORT, 'invalid service instance runtime_status') END;",
			...
			"CREATE TRIGGER IF NOT EXISTS trg_nodes_validate_insert BEFORE INSERT ON nodes FOR EACH ROW SELECT CASE WHEN NEW.is_configured NOT IN (0, 1) THEN RAISE(ABORT, 'invalid nodes.is_configured') WHEN NEW.is_online NOT IN (0, 1) THEN RAISE(ABORT, 'invalid nodes.is_online') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_nodes_validate_update BEFORE UPDATE OF is_configured, is_online ON nodes FOR EACH ROW SELECT CASE WHEN NEW.is_configured NOT IN (0, 1) THEN RAISE(ABORT, 'invalid nodes.is_configured') WHEN NEW.is_online NOT IN (0, 1) THEN RAISE(ABORT, 'invalid nodes.is_online') END;",
			"CREATE TRIGGER IF NOT EXISTS trg_service_image_states_validate_insert BEFORE INSERT ON service_image_states FOR EACH ROW SELECT CASE WHEN NEW.update_available NOT IN (0, 1) THEN RAISE(ABORT, 'invalid service_image_states.update_available') WHEN NEW.check_status NOT IN ('unknown', 'ok', 'error') THEN RAISE(ABORT, 'invalid service_image_states.check_status') END;",
