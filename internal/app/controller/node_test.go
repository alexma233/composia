...
func TestNodeQueryServiceGetNodeTasksReturnsFilteredTasks(t *testing.T) {
	...
	ctx := context.Background()
	if err := db.SyncDeclaredServices(ctx, map[string][]string{"alpha": {"main", "node-2"}}); err != nil {
		t.Fatalf("sync declared services: %v", err)
	}
	if err := db.SyncConfiguredNodes(ctx, []string{"main", "node-2"}); err != nil {
		t.Fatalf("sync configured nodes: %v", err)
	...
	client := controllerv1connect.NewNodeQueryServiceClient(httpServer.Client(), httpServer.URL, connect.WithInterceptors(rpcutil.NewStaticBearerAuthInterceptor("access-token")))
