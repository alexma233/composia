...
Composia 围绕一个核心理念构建：**你的 Compose 文件应该始终是事实的唯一来源。** 它在之上叠加多节点协作和运维可见性作为增强层——你的文件始终归你，CLI 工作流不被打断，随时可以脱离 Composia 而不丢失基础设施。
...
这也是为什么 Composia 坚持用纯文件保存期望状态，尽量贴近标准 Docker Compose 工作流，并把管理平台定位成增强层，而不是唯一入口。
...
- Composia 负责协调、校验、执行和汇总，而不是夺走操作者对系统的主权

## Composia 是什么

Composia 是一个面向自托管场景的 Docker Compose 管理平台，核心围绕这些能力展开：
...
- Composia 是面向自托管基础设施的 Compose 原生协调层
...
它保留了很多人真正想要的 Compose 原生、文件优先、操作者可控模型，同时补上了轻量 stack 管理器通常不提供的协调调度能力。

这正是 Composia 想站的位置。

## 如果你想要这些，Composia 更合适

- 一个多节点 Docker Compose 管理平台
...
**一个多节点 Docker Compose 协调层，与你的文件协作，而不是替代它们。**
