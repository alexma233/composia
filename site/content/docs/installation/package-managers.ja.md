...
パッケージまたはアーカイブは Composia バイナリのみをインストールします。Docker、Docker Compose、Git はインストールされません。
...
コントローラーまたはエージェントをコンテナ外で実行する場合、サービスマネージャーを使用して実行を維持します。

systemd コントローラーユニットの例:
...
ユーザー、グループ、設定パス、Docker ソケット権限は環境に合わせて調整してください。
