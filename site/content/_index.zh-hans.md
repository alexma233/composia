...
{{< hextra/hero-subtitle >}}
  为进阶用户打造的自托管编排系统。用纯文本定义服务，用 Git 管理，所有配置都基于文件、无平台绑定。
{{< /hextra/hero-subtitle >}}
</div>
...
{{< hextra/hero-button text="帮助" link="help-me" style="background-color: transparent; border: 1px solid var(--tw-prose-headings); color: var(--tw-prose-headings);" >}}
</div>
...
{{< hextra/feature-card
  title="标准文件，无平台绑定"
  subtitle="docker-compose.yaml + composia-meta.yaml，存储在 Git 仓库中。开放格式，所有配置都基于文件，随时可手动控制。"
  icon="lock-open"
