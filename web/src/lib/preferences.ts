...
export const availableAccentColors = ['blue', 'emerald', 'violet', 'rose', 'amber'] as const;
...
export const accentMetadata: Record<AccentColor, { label: string; preview: string }> = {
...
const initialThemeMode = normalizeThemeMode(
  typeof document === 'undefined' ? 'system' : document.documentElement.dataset.themeMode
);
const initialAccentColor = normalizeAccentColor(
  typeof document === 'undefined' ? 'blue' : document.documentElement.dataset.accent
);
...
export function initializePreferences() {
  const storedThemeMode = normalizeThemeMode(readStoredValue(storageKeys.themeMode));
  const storedAccentColor = normalizeAccentColor(readStoredValue(storageKeys.accentColor));
