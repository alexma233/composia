  ...
  nodes: {
    ...
    docker: {
      ...
      prune: {
        ...
        confirmAction: "执行清理",
        containersWarning:
          "要在节点 {nodeId} 上执行 `docker container prune -f` 吗？这会删除所有已停止的容器。",
        volumesWarning:
          "要在节点 {nodeId} 上执行 `docker volume prune -f` 吗？这会删除所有未使用的本地卷，并且可能移除你仍然需要的数据。",
        systemWarning:
          "要在节点 {nodeId} 上执行 `docker system prune -f` 吗？这会删除已停止容器、未使用网络、悬空镜像和构建缓存。",
        systemAllWarning:
          "要在节点 {nodeId} 上执行 `docker system prune -a -f` 吗？这还会删除未使用镜像，而不只是悬空镜像。",
        systemAllVolumesWarning:
          "要在节点 {nodeId} 上执行 `docker system prune -a --volumes -f` 吗？这还会删除未使用卷，并且可能永久丢失数据。",
      },
    },
  },
  services: {
