...
export function controllerProcedure(path: string): string {
  return `${controllerRpcBasePath}${path}`;
}
...
export async function loadNodes(): Promise<NodeSummary[]> {
  ...
  const response = await rpcCall<{
    ...
    config.baseUrl,
    config.token,
    controllerProcedure("/composia.controller.v1.NodeQueryService/ListNodes"),
    {},
  );
...
export async function loadTasks(
  ...
  const response = await rpcCall<{
    ...
    config.baseUrl,
    config.token,
    controllerProcedure("/composia.controller.v1.TaskService/ListTasks"),
    {
      page,
      pageSize,
...
export async function loadRepoFile(path: string): Promise<RepoFileContent> {
  ...
  return rpcCall<RepoFileContent>(
    config.baseUrl,
    config.token,
    controllerProcedure("/composia.controller.v1.RepoQueryService/GetRepoFile"),
    { path },
  );
}
  ...
  return {
    ...
    instances: (response.instances ?? []).map((instance) => ({
      ...
      pendingDeployRevision:
        instance.pendingDeployRevision ??
        instance.pending_deploy_revision ??
        "",
...
export async function loadNodeDetail(
  ...
  const response = await rpcCall<{
    ...
    config.baseUrl,
    config.token,
    controllerProcedure("/composia.controller.v1.NodeQueryService/GetNode"),
    { nodeId },
  );
  if (!response.node) {
    return null;
