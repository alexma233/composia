<script lang="ts">
  ...
  let runningServiceCount = $derived(
    data.dashboard?.system.runningServiceCount ?? 0,
  );
  ...
  let configuredNodeCount = $derived(
    data.dashboard?.system.configuredNodeCount ?? 0,
  );
  ...
  onMount(() =>
    startPolling(() => invalidate("app:dashboard"), {
      intervalMs: () =>
        hasActiveOperations([
          ...recentTasks.map((task) => task.status),
          ...recentBackups.map((backup) => backup.status),
        ])
          ? 5000
          : 30000,
    }),
    ...
    <div class="page-header">
      <div class="page-heading">
        <CardTitle class="page-title" level="1"
          >{$messages.dashboard.title}</CardTitle
        >
      </div>
    </div>
          ...
          <div class="flex items-center justify-between gap-3">
            <CardTitle class="section-title" level="2">
              <a
                ...
                href="/tasks">{$messages.dashboard.tasks}</a
              >
            </CardTitle>
        ...
        <CardContent>
          <div class="space-y-3">
            {#if recentTasks.length}
              {#each recentTasks as task (task.taskId)}
        ...
        <CardHeader>
          <div class="flex items-center justify-between gap-3">
            <CardTitle class="section-title" level="2">
              <a
                class="hover:text-foreground/80 transition-colors"
                href="/backups">{$messages.nav.backups}</a
              >
            </CardTitle>
        ...
        <CardContent>
          <div class="space-y-3">
            {#if recentBackups.length}
              {#each recentBackups as backup (backup.backupId)}
                <BackupCard {backup} />
              {/each}
            {:else if !data.error}
