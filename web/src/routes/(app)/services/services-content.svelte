<script lang="ts">
  ...
  onMount(() =>
    startPolling(() => invalidate("app:services"), { intervalMs: 15000 }),
  );
</script>
      ...
      <div class="page-header">
        <div class="page-heading">
          <CardTitle class="page-title" level="1"
            >{$messages.services.title}</CardTitle
          >
        </div>
                ...
                <form method="POST" action="?/create">
                  <div class="space-y-4">
                    ...
                    <input
                      type="hidden"
                      name="baseRevision"
                      value={data.repoHead.headRevision}
                    />
                    <div class="grid gap-2">
                      <label for="folder" class="text-sm font-medium"
                        >{$messages.services.folderName}</label
                      >
                      <Input
                        id="folder"
                        name="folder"
      ...
      {:else if data.services.length}
        <Table>
          <TableCaption class="sr-only"
            >{$messages.services.tableCaption}</TableCaption
          >
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {#each data.services as service (service.folder)}
              <ServiceRow {service} />
            {/each}
          </TableBody>
        </Table>
