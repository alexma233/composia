<script lang="ts">
  ...
  const pageSize = 20;
  const statusOptions: BackupStatusFilter[] = [
    "pending",
    "running",
    "awaiting_confirmation",
    "succeeded",
  ...
  let totalPages = $derived(
    data.totalCount > 0 ? Math.ceil(data.totalCount / pageSize) : 0,
  );
  ...
  const dataOptions = $derived(
    dataNameOptions(
      data.backups.map((backup) => backup.dataName),
      currentDataNames,
      excludedDataNames,
    ),
  );
  ...
  const statusSummary = $derived(
    selectionSummary(
      ...
      $messages.tasks.filters.allStatuses,
    ),
  );
  const serviceSummary = $derived(
    selectionSummary(
      ...
      serviceOptions,
      $messages.tasks.filters.allServices,
    ),
  );
  const nodeSummary = $derived(
    selectionSummary(
      ...
      excludedNodeIds,
      nodeOptions,
      $messages.tasks.filters.allNodes,
    ),
  );
  const dataSummary = $derived(
    selectionSummary(
      ...
      excludedDataNames,
      dataOptions,
      $messages.backups.filters.allDataNames,
    ),
  );
  ...
  const emptyMessage = $derived(
    activeFilterCount === 0
      ...
      : $messages.backups.noBackupsForFilter,
  );
  ...
  onMount(() =>
    startPolling(() => invalidate("app:backups"), {
      intervalMs: () =>
        hasActiveOperations(data.backups.map((backup) => backup.status))
          ? 5000
          : 30000,
    }),
  );
  ...
  function dataNameOptions(
    ...
    const values = Array.from(
      new Set(
        [...includeValues, ...excludeValues, ...loadedValues].filter(Boolean),
      ),
  ...
  function toggleFilterSelection<T extends string>(
    ...
    if (kind === "status") {
      if (mode === "include") {
        currentStatuses = toggleValue(
          currentStatuses as T[],
          value,
        ...
        excludedStatuses = toggleValue(
          excludedStatuses as T[],
          value,
        ) as BackupStatusFilter[];
    ...
    if (kind === "service") {
      if (mode === "include") {
        currentServiceNames = toggleValue(
          currentServiceNames as T[],
          value,
        ) as string[];
        excludedServiceNames = excludedServiceNames.filter(
          (entry) => entry !== value,
        ) as string[];
      } else {
        excludedServiceNames = toggleValue(
          excludedServiceNames as T[],
          value,
        ) as string[];
        currentServiceNames = currentServiceNames.filter(
          (entry) => entry !== value,
        ) as string[];
      }
    }
    if (kind === "node") {
      if (mode === "include") {
        ...
        excludedNodeIds = excludedNodeIds.filter(
          (entry) => entry !== value,
        ) as string[];
      } else {
        excludedNodeIds = toggleValue(
          excludedNodeIds as T[],
          value,
        ) as string[];
        currentNodeIds = currentNodeIds.filter(
          (entry) => entry !== value,
        ) as string[];
      }
    }
    if (kind === "data") {
      if (mode === "include") {
        currentDataNames = toggleValue(
          currentDataNames as T[],
          value,
        ) as string[];
        excludedDataNames = excludedDataNames.filter(
          (entry) => entry !== value,
        ) as string[];
      } else {
        excludedDataNames = toggleValue(
          excludedDataNames as T[],
          value,
        ) as string[];
        currentDataNames = currentDataNames.filter(
          (entry) => entry !== value,
        ) as string[];
      }
    }
  ...
  function isSelected(
    ...
    if (kind === "status")
      return mode === "include"
        ? currentStatuses.includes(value as BackupStatusFilter)
        : excludedStatuses.includes(value as BackupStatusFilter);
    if (kind === "service")
      return mode === "include"
        ? currentServiceNames.includes(value)
        : excludedServiceNames.includes(value);
    if (kind === "node")
      return mode === "include"
        ? currentNodeIds.includes(value)
        : excludedNodeIds.includes(value);
    return mode === "include"
      ? currentDataNames.includes(value)
      : excludedDataNames.includes(value);
  }
  ...
  function hasModeValues(kind: FilterKind, mode: FilterMode): boolean {
    ...
    if (kind === "service")
      return mode === "include"
        ...
        : excludedServiceNames.length > 0;
    if (kind === "node")
      return mode === "include"
        ? currentNodeIds.length > 0
        : excludedNodeIds.length > 0;
    return mode === "include"
      ? currentDataNames.length > 0
      : excludedDataNames.length > 0;
  }
  ...
  function pageUrl(
    ...
    dataNames: string[],
    excludeDataNames: string[],
  ): string {
    return buildFilterPageUrl(currentPath, page, {
      status: statuses,
      excludeStatus: excludeStatuses,
      serviceName: serviceNames,
      excludeServiceName: excludeServiceNames,
      nodeId: nodeIds,
      excludeNodeId: excludeNodeIds,
      dataName: dataNames,
      excludeDataName: excludeDataNames,
    });
  }
  ...
  $effect(() => {
    if (
      currentPage === data.page &&
      filterValuesEqual(currentStatuses, data.status) &&
      filterValuesEqual(excludedStatuses, data.excludeStatus) &&
      filterValuesEqual(currentServiceNames, data.serviceName) &&
      filterValuesEqual(excludedServiceNames, data.excludeServiceName) &&
      filterValuesEqual(currentNodeIds, data.nodeId) &&
      filterValuesEqual(excludedNodeIds, data.excludeNodeId) &&
      filterValuesEqual(currentDataNames, data.dataName) &&
      filterValuesEqual(excludedDataNames, data.excludeDataName)
    ) {
      return;
    }
      ...
      <div class="page-header">
        <div class="page-heading">
          <CardTitle class="page-title" level="1"
            >{$messages.backups.title}</CardTitle
          >
        </div>
          ...
          <Popover.Root bind:open={filterOpen}>
            ...
            >
              <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    >
                      <div class="flex border-b p-1 gap-1">
                        <Button
                          type="button"
                          size="sm"
                          variant={statusMode === "include"
                            ? "default"
                            : "ghost"}
                          onclick={() => {
                            statusMode = "include";
                          }}>{modeLabel("include")}</Button
                        >
                        <Button
                          type="button"
                          size="sm"
                          variant={statusMode === "exclude"
                            ? "default"
                            : "ghost"}
                          onclick={() => {
                            statusMode = "exclude";
                          }}>{modeLabel("exclude")}</Button
                        >
                      </div>
                      <Command.Root>
                        <Command.Input
                          placeholder={$messages.tasks.filters
                            .searchStatusPlaceholder}
                        />
                        <Command.List>
                          <Command.Empty
                            >{$messages.tasks.filters
                              .noStatusesFound}</Command.Empty
                          >
                          <Command.Group>
                            <Command.Item
                              value="__all__"
                              onSelect={() => {
                                clearModeValues("status", statusMode);
                                closeAndFocus("status");
                              }}
                            >
                              <span
                                >{statusMode === "include"
                                  ? $messages.tasks.filters.clearIncluded
                                  : $messages.tasks.filters.clearExcluded}</span
                              >
                              <Check
                                class={cn(
                                  "ml-auto size-4",
                                  hasModeValues("status", statusMode) &&
                                    "text-transparent",
                                )}
                              />
                            </Command.Item>
                            {#each statusOptions as status}
                              ...
                              >
                                ...
                                <Check
                                  class={cn(
                                    "ml-auto size-4",
                                    !isSelected("status", statusMode, status) &&
                                      "text-transparent",
                                  )}
                                />
                              </Command.Item>
                ...
                <div class="space-y-2">
                  <div class="text-sm text-muted-foreground">
                    {$messages.tasks.filters.serviceName}
                  </div>
                  <Popover.Root bind:open={serviceSelectOpen}>
                    <Popover.Trigger
                      bind:ref={serviceTriggerRef}
                      class={buttonVariants({
                        variant: "outline",
                        class: "w-full justify-between font-normal",
                      })}
                    ...
                    >
                      ...
                      <Command.Root>
                        ...
                        <Command.List>
                          <Command.Empty
                            >{$messages.tasks.filters
                              .noServicesFound}</Command.Empty
                          >
                          <Command.Group>
                            <Command.Item
                              value="__all__"
                              onSelect={() => {
                                clearModeValues("service", serviceMode);
                                closeAndFocus("service");
                              }}
                            >
                              <span
                                >{serviceMode === "include"
                              ...
                              <Check
                                class={cn(
                                  "ml-auto size-4",
                                  hasModeValues("service", serviceMode) &&
                                    "text-transparent",
                                )}
                              />
                            </Command.Item>
                ...
                <div class="space-y-2">
                  <div class="text-sm text-muted-foreground">
                    {$messages.tasks.filters.nodeId}
                  </div>
                  <Popover.Root bind:open={nodeSelectOpen}>
                    <Popover.Trigger
                      bind:ref={nodeTriggerRef}
                      class={buttonVariants({
                        variant: "outline",
                        class: "w-full justify-between font-normal",
                      })}
                    ...
                    >
                      <div class="flex border-b p-1 gap-1">
                        <Button
                          type="button"
                          size="sm"
                          variant={nodeMode === "include" ? "default" : "ghost"}
                          onclick={() => {
                            nodeMode = "include";
                          }}>{modeLabel("include")}</Button
                        >
                        <Button
                          type="button"
                          size="sm"
                          variant={nodeMode === "exclude" ? "default" : "ghost"}
                          onclick={() => {
                            nodeMode = "exclude";
                          }}>{modeLabel("exclude")}</Button
                        >
                      ...
                      <Command.Root>
                        ...
                        <Command.List>
                          <Command.Empty
                            >{$messages.tasks.filters
                              .noNodesFound}</Command.Empty
                          >
                          <Command.Group>
                            <Command.Item
                              value="__all__"
                              onSelect={() => {
                                clearModeValues("node", nodeMode);
                                closeAndFocus("node");
                              }}
                            >
                              <span
                            ...
                            {#each nodeOptions as node (node.value)}
                              <Command.Item
                                value={`${node.label} ${node.secondary ?? ""}`}
                                onSelect={() => {
                                  toggleFilterSelection(
                                    "node",
                                    nodeMode,
                              ...
                              >
                                ...
                                <Check
                                  class={cn(
                                    "ml-auto size-4",
                                    !isSelected("node", nodeMode, node.value) &&
                                      "text-transparent",
                                  )}
                                />
                              </Command.Item>
                ...
                <div class="space-y-2">
                  <div class="text-sm text-muted-foreground">
                    {$messages.backups.dataName}
                  </div>
                  <Popover.Root bind:open={dataSelectOpen}>
                    <Popover.Trigger
                      bind:ref={dataTriggerRef}
                      class={buttonVariants({
                        variant: "outline",
                        class: "w-full justify-between font-normal",
                      })}
                    ...
                    >
                      <div class="flex border-b p-1 gap-1">
                        <Button
                          type="button"
                          size="sm"
                          variant={dataMode === "include" ? "default" : "ghost"}
                          onclick={() => {
                            dataMode = "include";
                          }}>{modeLabel("include")}</Button
                        >
                        <Button
                          type="button"
                          size="sm"
                          variant={dataMode === "exclude" ? "default" : "ghost"}
                          onclick={() => {
                            dataMode = "exclude";
                          }}>{modeLabel("exclude")}</Button
                        >
                      ...
                      <Command.Root>
                        <Command.Input
                          placeholder={$messages.backups.filters
                            .searchDataPlaceholder}
                        />
                        <Command.List>
                          <Command.Empty
                            >{$messages.backups.filters
                              .noDataFound}</Command.Empty
                          >
                          <Command.Group>
                            <Command.Item
                              value="__all__"
                              onSelect={() => {
                                clearModeValues("data", dataMode);
                                closeAndFocus("data");
                              }}
                            >
                              <span
      ...
      {:else if data.backups.length}
        <Table>
          <TableCaption class="sr-only"
            >{$messages.backups.tableCaption}</TableCaption
          >
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {#each data.backups as backup (backup.backupId)}
              <BackupRow {backup} />
            {/each}
          </TableBody>
        </Table>
