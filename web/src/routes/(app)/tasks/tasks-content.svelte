<script lang="ts">
  ...
  const typeSummary = $derived(
    selectionSummary(
      currentTypes,
      excludedTypes,
      taskTypeOptions.map((type) => ({ value: type, label: typeLabel(type) })),
      $messages.tasks.filters.allTypes,
    ),
  );
  ...
  onMount(() =>
    startPolling(() => invalidate("app:tasks"), {
      intervalMs: () =>
  ...
  function selectionSummary(
    ...
    emptyLabel = $messages.tasks.filters.allStatuses,
  ): string {
  ...
  function clearModeValues(
    kind: "status" | "service" | "node" | "type",
    mode: FilterMode,
  ) {
    if (kind === "status") {
  ...
  function toggleFilterSelection<T extends string>(
    ...
    if (kind === "type") {
      if (mode === "include") {
        currentTypes = toggleValue(
          currentTypes as T[],
          value,
        ) as TaskTypeFilter[];
        excludedTypes = excludedTypes.filter(
          (entry) => entry !== value,
        ) as TaskTypeFilter[];
      } else {
        excludedTypes = toggleValue(
          excludedTypes as T[],
          value,
        ) as TaskTypeFilter[];
  ...
  function isSelected(
    ...
    if (kind === "status")
      return mode === "include"
        ? currentStatuses.includes(value as TaskStatusFilter)
        : excludedStatuses.includes(value as TaskStatusFilter);
    if (kind === "service")
      return mode === "include"
    ...
    return mode === "include"
      ? currentTypes.includes(value as TaskTypeFilter)
      : excludedTypes.includes(value as TaskTypeFilter);
  }
  ...
  $effect(() => {
    if (
      ...
      filterValuesEqual(currentTypes, data.type) &&
      filterValuesEqual(excludedTypes, data.excludeType)
    ) {
      return;
    }
      ...
      <div class="page-header">
        <div class="page-heading">
          <CardTitle class="page-title" level="1"
            >{$messages.tasks.taskHistory}</CardTitle
          >
        </div>
          ...
          <Popover.Root bind:open={filterOpen}>
            ...
            >
              <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div class="space-y-2">
                  <div class="text-sm text-muted-foreground">
                    {$messages.tasks.filters.type}
                  </div>
                  <Popover.Root bind:open={typeSelectOpen}>
                    <Popover.Trigger
                      bind:ref={typeTriggerRef}
                      class={buttonVariants({
                        variant: "outline",
                        class: "w-full justify-between font-normal",
                      })}
                    ...
                    >
                      <div class="flex border-b p-1 gap-1">
                        <Button
                          type="button"
                          size="sm"
                          variant={typeMode === "include" ? "default" : "ghost"}
                          onclick={() => {
                            typeMode = "include";
                          }}>{modeLabel("include")}</Button
                        >
                        <Button
                          type="button"
                          size="sm"
                          variant={typeMode === "exclude" ? "default" : "ghost"}
                          onclick={() => {
                            typeMode = "exclude";
                          }}>{modeLabel("exclude")}</Button
                        >
                      ...
                      <Command.Root>
                        ...
                        <Command.List>
                          <Command.Empty
                            >{$messages.tasks.filters
                              .noTypesFound}</Command.Empty
                          >
                          <Command.Group>
                            <Command.Item
                              value="__all__"
                              onSelect={() => {
                                clearModeValues("type", typeMode);
                                closeAndFocus("type");
                              }}
                            >
                              <span
                            ...
                            {#each taskTypeOptions as type}
                              <Command.Item
                                value={`${typeLabel(type)} ${type}`}
                                onSelect={() => {
                                  toggleFilterSelection("type", typeMode, type);
                                }}
                              >
    ...
    <CardContent>
      ...
      {:else}
        <div class="empty-state">
          {#if data.status.length === 0 && data.excludeStatus.length === 0 && data.serviceName.length === 0 && data.excludeServiceName.length === 0 && data.nodeId.length === 0 && data.excludeNodeId.length === 0 && data.type.length === 0 && filterValuesEqual(data.excludeType, defaultExcludedTypes)}
