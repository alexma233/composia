...
import { defaultServiceFilePath, normalizeServiceRelativePath } from '$lib/service-workspace';
import {
  loadServiceFileTree,
...
export const load: PageServerLoad = async ({ params, url }) => {
  ...
  try {
    const [workspaces, repoHead] = await Promise.all([loadServiceWorkspaces(), loadRepoHead()]);
