...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  try {
    const file = await loadServiceWorkspaceFile(params.name, normalizeServiceRelativePath(path));
    return json({ file });
  } catch (error) {
    return json(
      { error: error instanceof Error ? error.message : 'Failed to load workspace file.' },
      { status: 400 }
    );
  }
};
