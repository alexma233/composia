...
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    ...
    switch (payload.action) {
      ...
      case 'move':
        if (!payload.sourcePath || !payload.destinationPath) {
          return json({ error: 'Source and destination paths are required.' }, { status: 400 });
        }
      ...
      case 'delete':
        ...
        break;
      default:
        return json({ error: 'Unsupported file management action.' }, { status: 400 });
    }
    ...
    return json(
      { error: error instanceof Error ? error.message : 'Failed to update workspace files.' },
      { status: 400 }
    );
  }
};
