...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const excludeServiceName = parseTextFilters(url.searchParams.getAll("excludeServiceName"));
  const excludeNodeId = parseTextFilters(url.searchParams.getAll("excludeNodeId"));
  const rawExcludeType = parseTextFilters(url.searchParams.getAll("excludeType"));
  const excludeType = rawExcludeType.length > 0 ? rawExcludeType : defaultExcludedTaskTypes;
    ...
    const [servicesResult, nodes, result] = await Promise.all([
      ...
      loadNodes(),
		loadTasks(page, 20, {
		  ...
		  excludeServiceName: excludeServiceName.length ? excludeServiceName : undefined,
