<script lang="ts">
  ...
  function applyContainerRawJson(rawJson: string | null) {
    ...
    try {
      ...
      parseError =
        error instanceof Error ? error.message : $messages.error.parseFailed;
      containerData = null;
    }
  }
  ...
  async function startLogStream() {
    ...
    logs = "";
    logsError = "";
    logsState = "connecting";

    try {
      const response = await fetch(
        `/nodes/${encodeURIComponent(data.nodeId)}/docker/containers/${encodeURIComponent(data.containerId)}/logs?tail=${encodeURIComponent(logTail)}`,
        {
      ...
      if (!response.ok || !response.body) {
        throw new Error(
          `${$messages.docker.containers.logs.loadFailed}: ${response.status}`,
        );
      }
      ...
      logsError =
        error instanceof Error
          ? error.message
          : $messages.docker.containers.logs.loadFailed;
    } finally {
  ...
  async function queueContainerAction(action: "start" | "stop" | "restart") {
    ...
    try {
      const response = await fetch(
        `/nodes/${encodeURIComponent(data.nodeId)}/docker/containers/${encodeURIComponent(data.containerId)}/actions/${action}`,
        {
          method: "POST",
      ...
      toast.success(
        $messages.docker.containers.actionQueued
          .replace("{action}", actionLabel(action))
          .replace("{taskId}", payload.taskId?.slice(0, 12) ?? "task"),
      );
      if (payload.taskId) {
    ...
    try {
      ...
      socket.onmessage = (event) => {
          ...
          try {
            ...
            if (message.type === "closed") {
              appendTerminalNotice(
                $messages.docker.containers.terminal.sessionClosed,
              );
            }
          } catch {
        ...
        const text = new TextDecoder().decode(
          event.data instanceof ArrayBuffer ? event.data : new Uint8Array(),
        );
      ...
      <CardContent>
        ...
        {:else if parseError}
          <Alert variant="destructive">
            ...
            <AlertDescription
              >{$messages.error.parseFailed}: {parseError}</AlertDescription
            >
          </Alert>
            ...
            <div class="mb-4 overflow-x-auto pb-1 scrollbar-none">
              <TabsList class="min-w-max">
                <TabsTrigger value="info"
                  >{$messages.docker.containers.info}</TabsTrigger
                >
                <TabsTrigger value="logs"
                  >{$messages.docker.containers.logsLabel}</TabsTrigger
                >
                <TabsTrigger value="terminal"
                  >{$messages.docker.containers.terminalLabel}</TabsTrigger
                >
                <TabsTrigger value="config"
                  >{$messages.docker.containers.config}</TabsTrigger
                >
                <TabsTrigger value="env"
                  >{$messages.docker.containers.environment}</TabsTrigger
                >
                <TabsTrigger value="network"
                  >{$messages.docker.containers.network}</TabsTrigger
                >
                <TabsTrigger value="volumes"
                  >{$messages.docker.containers.volumes}</TabsTrigger
                >
                <TabsTrigger value="labels"
                  >{$messages.docker.containers.labels}</TabsTrigger
                >
                <TabsTrigger value="raw"
                  >{$messages.docker.containers.json}</TabsTrigger
                >
              </TabsList>
            </div>
              ...
              <div class="grid gap-4 md:grid-cols-2">
                  ...
                  <CardContent class="space-y-2 text-sm">
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.id}</span
                      >
                      <code class="text-xs bg-muted px-1 py-0.5 rounded"
                        >{containerData.Id?.substring(0, 12)}</code
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.common.name}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Name?.replace(/^\//, "") || "-"}</span
                      >
                    </div>
                    <div
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.image}</span
                      >
                      <span
                        class="break-all sm:max-w-[20rem] sm:text-right"
                        title={containerData.Config?.Image}
                      >
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.platform}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Platform || "-"}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.driver}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Driver || "-"}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                ...
                <Card>
                  ...
                  <CardContent class="space-y-2 text-sm">
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.running}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.State?.Running
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    {#if containerData.State?.Running}
                      ...
                      >
                        <span class="text-muted-foreground"
                          >{$messages.docker.containers.uptime}</span
                        >
                        <span class="sm:text-right"
                          >{formatDuration(
                    ...
                    {:else}
                      ...
                      >
                        <span class="text-muted-foreground"
                          >{$messages.docker.containers.exitCode}</span
                        >
                        <span class="sm:text-right"
                          >{containerData.State?.ExitCode ?? "-"}</span
                        >
                      </div>
                      <div
                        class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.restartCount}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.RestartCount || 0}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.oomKilled}</span
                      >
                      <span class="sm:text-right"
              ...
              <Card>
                ...
                <CardContent class="space-y-2 text-sm">
                  <div class="grid gap-4 md:grid-cols-3">
                    <div>
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.command}</span
                      >
                      <code
                        class="block mt-1 text-xs bg-muted p-2 rounded break-all"
                      >
                    ...
                    <div>
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.entrypoint}</span
                      >
                      <code
                        class="block mt-1 text-xs bg-muted p-2 rounded break-all"
                      >
                    ...
                    <div>
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.workingDir}</span
                      >
                      <code
                        class="block mt-1 text-xs bg-muted p-2 rounded break-all"
                      >
                ...
                <CardHeader class="pb-3">
                  ...
                  >
                    <div>
                      ...
                      >
                      <CardDescription
                        >{$messages.docker.containers.logs
                    ...
                    >
                      ...
                      <Button
                        ...
                        disabled={logsState === "connecting"}
                        >{$messages.docker.containers.logs.refresh}</Button
                      >
                    </div>
                  </div>
                </CardHeader>
            ...
            <TabsContent value="terminal" class="space-y-4">
              <Card>
                <CardHeader class="pb-3">
                  ...
                  >
                  <CardDescription
                    >{$messages.docker.containers.terminal
                      .description}</CardDescription
                  >
                </CardHeader>
                <CardContent class="space-y-4">
                  <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    ...
                    >
                      {terminalSocket
                        ...
                        : $messages.docker.containers.terminal.connect}
                    </Button>
                    <Button
                      class="w-full sm:w-auto"
                      variant="outline"
                      onclick={disconnectTerminal}
                      disabled={!terminalSocket}
                      >{$messages.docker.containers.terminal.disconnect}</Button
                    >
                  </div>
                  ...
                  <XtermSurface
                    ...
                    emptyText={terminalConnecting
                      ? $messages.docker.containers.terminal.connecting
                      : terminalSocket
                        ? ""
                  ...
                  {#if terminalSessionId}
                    <div class="text-xs text-muted-foreground">
                      {$messages.docker.containers.session}
                      {terminalSessionId}
                    </div>
                  {/if}
                </CardContent>
              ...
              <div class="grid gap-4 md:grid-cols-2">
                <Card>
                  ...
                  <CardContent class="space-y-2 text-sm">
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.hostname}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Config?.Hostname || "-"}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.domainname}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Config?.Domainname || "-"}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.user}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.Config?.User ||
                          $messages.common.root}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.attachStdin}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.AttachStdin
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.attachStdout}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.AttachStdout
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.attachStderr}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.AttachStderr
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.tty}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.Tty
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.openStdin}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.OpenStdin
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.stdinOnce}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.Config?.StdinOnce
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                ...
                <Card>
                  ...
                  <CardContent class="space-y-2 text-sm">
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.networkMode}</span
                      >
                      <span class="break-all sm:text-right"
                        >{containerData.HostConfig?.NetworkMode ||
                          $messages.common.defaultValue}</span
                      >
                    </div>
                    <div
                      class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.privileged}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.HostConfig?.Privileged
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.autoRemove}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.HostConfig?.AutoRemove
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    ...
                    >
                      <span class="text-muted-foreground"
                        >{$messages.docker.containers.readonlyRootfs}</span
                      >
                      <span class="sm:text-right"
                        >{containerData.HostConfig?.ReadonlyRootfs
                          ? $messages.docker.containers.yes
                          : $messages.docker.containers.no}</span
                      >
                    </div>
                    {#if containerData.HostConfig?.Memory}
                      ...
                      >
                        <span class="text-muted-foreground"
                          >{$messages.docker.containers.memoryLimit}</span
                        >
                        <span class="sm:text-right"
                          >{formatBytes(containerData.HostConfig.Memory)}</span
                        >
                      </div>
                    {/if}
                    {#if containerData.HostConfig?.CpuShares}
                      ...
                      >
                        <span class="text-muted-foreground"
                          >{$messages.docker.containers.cpuShares}</span
                        >
                        <span class="sm:text-right"
                          >{containerData.HostConfig.CpuShares}</span
                        >
                      </div>
                    {/if}
                ...
                <CardContent>
                  {#if containerData.Config?.Env && containerData.Config.Env.length > 0}
                    <div class="space-y-1">
                      {#each containerData.Config.Env as env}
                        ...
                        <div
                          class="flex flex-col gap-1 border-b border-border/50 py-1.5 font-mono text-xs last:border-0 sm:flex-row sm:gap-2"
                        >
                          <span class="text-foreground font-medium shrink-0"
                            >{key}=</span
                          >
                  ...
                  {:else}
                    <div class="text-sm text-muted-foreground">
                      {$messages.docker.containers.noEnvVars}
                    </div>
                  {/if}
                </CardContent>
              </Card>
            ...
            <TabsContent value="network" class="space-y-4">
              <Card>
                ...
                <CardContent>
                  {#if containerData.HostConfig?.PortBindings && Object.keys(containerData.HostConfig.PortBindings).length > 0}
                    <div class="space-y-2">
                      {#each Object.entries(containerData.HostConfig.PortBindings) as [containerPort, bindings]}
                        ...
                        <div
                          class="flex flex-col gap-2 border-b border-border/50 py-2 text-sm last:border-0 sm:flex-row sm:items-center sm:gap-3"
                        >
                  ...
                  {:else}
                    <div class="text-sm text-muted-foreground">
                      {$messages.docker.containers.noPortBindings}
                    </div>
                  {/if}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {:else}
                    <div class="text-sm text-muted-foreground">
                      {$messages.docker.containers.noExposedPorts}
                    </div>
                  {/if}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent class="space-y-2 text-sm">
                  ...
                  >
                    <span class="text-muted-foreground"
                      >{$messages.docker.containers.gateway}</span
                    >
                    <span class="break-all sm:text-right"
                      >{containerData.NetworkSettings?.Gateway || "-"}</span
                    >
                  </div>
                  <div
                    class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <span class="text-muted-foreground"
                      >{$messages.docker.containers.ipAddress}</span
                    >
                    <span class="break-all sm:text-right"
                      >{containerData.NetworkSettings?.IPAddress || "-"}</span
                    >
                  </div>
                  <div
                    class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <span class="text-muted-foreground"
                      >{$messages.docker.containers.macAddress}</span
                    >
                    <span class="break-all sm:text-right"
                      >{containerData.NetworkSettings?.MacAddress || "-"}</span
                    >
                  </div>
                </CardContent>
              </Card>
            ...
            <TabsContent value="volumes" class="space-y-4">
              <Card>
                ...
                <CardContent>
                  {#if containerData.Mounts && containerData.Mounts.length > 0}
                    <div class="space-y-3">
                      {#each containerData.Mounts as mount}
                        <div
                          class="border-b border-border/50 last:border-0 pb-3 last:pb-0"
                        >
                          ...
                          <div class="grid gap-1 text-sm">
                            ...
                            >
                              <span class="text-muted-foreground w-16 shrink-0"
                                >{$messages.common.source}:</span
                              >
                              <code
                                class="text-xs bg-muted px-1 py-0.5 rounded break-all"
                                >{mount.Source}</code
                              >
                            </div>
                            <div
                            ...
                            >
                              <span class="text-muted-foreground w-16 shrink-0"
                                >{$messages.common.target}:</span
                              >
                              <code
                                class="text-xs bg-muted px-1 py-0.5 rounded break-all"
                                >{mount.Destination}</code
                              >
                            </div>
                          </div>
                        </div>
                  ...
                  {:else}
                    <div class="text-sm text-muted-foreground">
                      {$messages.docker.containers.noMounts}
                    </div>
                  {/if}
                </CardContent>
              </Card>
            ...
            <TabsContent value="labels" class="space-y-4">
              <Card>
                ...
                <CardContent>
                  {#if containerData.Config?.Labels && Object.keys(containerData.Config.Labels).length > 0}
                    <div class="space-y-1">
                      {#each Object.entries(containerData.Config.Labels) as [key, value]}
                        <div
                          class="flex flex-col gap-1 border-b border-border/50 py-1.5 text-sm last:border-0 sm:flex-row sm:gap-2"
                        >
                          <code
                            class="text-xs bg-muted px-1 py-0.5 rounded shrink-0"
                            >{key}</code
                          >
                          <span class="text-muted-foreground break-all"
                            >{value}</span
                  ...
                  {:else}
                    <div class="text-sm text-muted-foreground">
                      {$messages.docker.containers.noLabels}
                    </div>
                  {/if}
                </CardContent>
              </Card>
